...
def iterate(exposure_indices,target, v_alpha, h_alpha, v_beta, h_beta):
    ...
    logpoints = np.arange(0,max_iter,10)
    #logpoints = np.array([max_iter+1])
    checkpoints = np.arange(0,max_iter,1)
    ...
    for i in range(population_size):
        ...
        for j in range(population.shape[0]):
            population[j, i] = np.random.randint(-start.max()/200,start.max()/200)
    population = check_limits(population)
    sigma_start = start.max()/50
    sigma = sigma_start
    starttime = time.time()
    while True: #for i in range(max_iter):
        ...
        if i in logpoints:
            print("{0:7d}: fitness: {1:5.3f}, sigma: {2:5.5f}, std_err: {3:5.5f}, slope: {4:5.5f}, var: {5:5.5f}".format(i, np.sqrt(fitness[0]), sigma,std_err,slope,variance))
